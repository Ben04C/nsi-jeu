...
inventaire = pygame.image.load("assets/UI/inventaire.png").convert_alpha()
...
explanation1= dialogueFont.render("What is going on here… (Press any key to continue.)", True, (255, 255, 255))
...
while running:
    ...
    if progress == 1:
        oftenusedD1()
        toprint = dialogueFont.render(" Why is it always me getting called at 2 AM?", True, (255, 255, 255))#Ceci sont les paramètres du texte à afficher
    ...
    if progress == 2:
        oftenusedD1()
        toprint = dialogueFont.render("Ah well… I guess it is my job after all… ", True, (255, 255, 255))
        screen.blit(toprint, (250,500))
        toprint = dialogueFont.render("Well then, let’s see what’s going on.", True, (255, 255, 255))
        pygame.display.update()
        checkanykey()

    if progress == 3:
        oftenusedD1()
        toprint = dialogueFont.render("I see a Police officer standing at the end of the road.", True, (255, 255, 255))
        screen.blit(toprint, (250, 500))
        pygame.display.update()
        checkanykey()
    ...
    if progress == 4:
        oftenusedD1()
        toprint = dialogueFont.render("Let’s see if he has something to tell us.", True, (255, 255, 255))
        screen.blit(toprint, (250, 500))
        pygame.display.update()
        checkanykey()
    ...
    for event in pygame.event.get():
        inventory=False
        if event.type == pygame.KEYDOWN:
            if event.key == K_TAB and inventory==False:
                inventory = True
                screen.blit(inventaire, (0, 0))
                pygame.display.update()
            if event.key == K_TAB and inventory==True:
                inventory = False
                screen.blit(inventaire, (0, 0))
                pygame.display.update()